...
```{r, echo = TRUE}
# let's use the herbivore.traits dataset for our first plots
...
.pull-left[
- aesthetics in ggplot (specified with the aes() argument) clarify which columns hold the infromation and how you want to use it
...
- this is done using the fact_grid() or facet_wrap() argument
...
### b) Change the points to be black triangles with size 4
...
p4 <- ggplot(data = herbivore.traits, 
             aes(x = eyediameter, 
                 y = size)) +
  geom_violin(draw_quantiles = c(0.2, 0.5, 0.8))
```
]
...
p4 <- ggplot(data = herbivore.traits, 
             aes(x = eyediameter, 
                 y = size,
                 fill = size)) +
  geom_violin(draw_quantiles = c(0.2, 0.5, 0.8), alpha = 0.5)
```
]
...
---
## Barplots and caterpillar plots

.pull-left[

- plot means and their uncertainties

- barplots with error bars or caterpillar plots provide a good option `r emo::ji("bug")`
...
p6 <- ggplot(data = herbivore.traits, 
             aes(x = family, 
                 ...
                 fill = family)) +
  geom_violin(draw_quantiles = c(0.05, 0.5, 0.95)) + 
  geom_jitter(width = 0.1, color = "white", alpha = 0.5) +
